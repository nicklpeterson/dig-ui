...
import { QuestionTable, RecordTable, ResultSection } from "./ResultSection";
...
function formatRawOutput(response: DnsResponse): string {
	...
	const flags = [
		...
		.join(" ");
	lines.push(
		`;; flags: ${flags}; QUERY: ${response.question.length}, ANSWER: ${response.answer.length}, AUTHORITY: ${response.authority.length}, ADDITIONAL: ${response.additional.length}`,
	);
	lines.push("");
...
export function ResultsPanel({ response }: ResultsPanelProps) {
	return (
		<Tabs defaultValue="formatted" className="w-full">
			<TabsList
				variant="line"
				className="bg-transparent border-b border-border rounded-none w-full md:w-50 justify-start gap-0 px-0 h-auto"
			>
				<TabsTrigger
					value="formatted"
			...
			<TabsContent value="raw" className="mt-4">
				<div className="relative group">
					...
					<pre
						className="bg-muted p-4 text-sm font-mono leading-relaxed overflow-x-auto whitespace-pre"
